...
func main() {
	...
	expectedSymbols := []string{
		...
		"GetFunc", "Invalidating", "Key", "KeyBuilder", "KeyedLoaderFunc",
			...
			"NewPrefixKeyBuilder", "NewPrefixKeyBuilderWithSeparator", "NewRedis", "PrefixKeyBuilder", "RedisOption",
	...
	sourceChecks := []struct {
		...
		},
		{
			...
			terms: []string{
					...
					"separator: \":\"", "separator: separator", "return strings.Join(keyParts, k.separator)",
...
func publicDocSurfaceTerms() []string {
	return []string{
		...
		"`cache.EvictionPolicyFIFO`", "`cache.Key(keyParts ...string) string`",
			...
			"`cache.NewPrefixKeyBuilderWithSeparator(prefix, separator string) *cache.PrefixKeyBuilder`",
