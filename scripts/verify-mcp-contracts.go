	...
	checks := []struct {
		...
		},
		{
			...
			docTerms: []string{
				...
				"mcp: cannot generate schema for nil value", "mcp: generate schema:",
				"required", "nullable",
		...
		},
		{
			...
			sourceTerms: []string{
				...
				"return \"vef-mcp-server\"", "return version.VEFVersion",
			},
			docTerms: []string{
				"mcp.ServerInfo", "vef.app.name", "vef-mcp-server", "version.VEFVersion",
			},
...
func verifyContractLedger(contracts contractLedger, sourceRoot string) []string {
	...
	for _, entry := range contracts.Entries {
		...
		for _, term := range []string{
			...
			"SchemaOf(nil)",
			"MustSchemaOf(nil)",
			"mcp: cannot generate schema for nil value",
			"mcp: generate schema:",
			`jsonschema:"required"`,
			"$schema",
			"PrincipalAnonymous",
