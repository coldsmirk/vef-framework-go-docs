...
All user-facing APIs must be reviewed without omissions. The audit is source-driven and has three required layers:

- [Public API Index](./public-api-index) lists every exported top-level symbol, exported field, and exported method from non-`internal/` source packages
- [Runtime API Index](./runtime-api-index) lists user-facing runtime contracts such as routes, CLI commands and flags, config keys, JSON fields, event topics, tag grammars, MCP contracts, and error codes
- `scripts/api-contract-ledger.json` records the package-level semantic review and pins the exact reviewed top-level/field/method counts and fingerprint for each package

When the framework public surface changes, update the affected topical docs and regenerate or verify the audit artifacts:
