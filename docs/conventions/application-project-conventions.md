...
:::tip
Read [Project Structure](../getting-started/project-structure) first for the baseline module layout. Read this page for the naming rules, API naming rules, and project-level conventions that every VEF application MUST follow.
:::
...
- [Routing](../building-apis/routing) for request identifier structure and endpoint exposure
- [Custom Handlers](../building-apis/custom-handlers) for RPC action to handler method resolution
- [Models](../data-access/models) for field, JSON tag, and column naming interplay
