...
`i18n.GetSupportedLanguages()` returns a copy of the supported-language slice, so modifying the returned slice does not mutate framework state. The shipped `en.json` and `zh-CN.json` catalogs always contain the same set of message IDs (currently 243).
