...
| `cache.NewInvalidating[T](loader cache.KeyedLoaderFunc[T], logger logx.Logger, opts ...cache.MemoryOption) *cache.Invalidating[T]` | creates an invalidating cache; callers must pass a non-nil loader and logger |
...
`cache.NewInvalidating[T](loader, logger, opts...)` wraps an internal `cache.NewMemory[T]()` instance. It accepts `cache.MemoryOption` values (for example `cache.WithMemMaxSize` or `cache.WithMemDefaultTTL`) but no Redis options. Pass a non-nil `cache.KeyedLoaderFunc[T]` and a non-nil `logx.Logger`; the constructor does not validate them before later method calls.
