...
`i18n.GetSupportedLanguages()` returns a copy of the supported-language slice, so modifying the returned slice does not mutate framework state. The current shipped `en.json` and `zh-CN.json` catalogs contain the same 160 message IDs.
