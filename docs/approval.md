...
RPC calls use the standard envelope documented in [API](./building-apis/api): `resource`,
...
`instance.formData` carries submitted form data (already stripped of fields
the viewer may not see), `instance.businessRef` is the opaque business
reference when the flow is business-bound, and `fieldPermissions` is the
viewer-scoped field interactivity map the client applies verbatim.
...
| `my.InstanceDetail` | `instance`, `formSchema`, `timeline`, `flowGraph`, `availableActions`, `fieldPermissions` |
...
Business binding connects the approval flow to your domain tables via `BusinessTable`, `BusinessPKField`, and `BusinessStatusField`, plus the optional `BusinessInstanceIDField`, `BusinessStartedAtField`, and `BusinessFinishedAtField` linkage columns (see [Business Write-Back Linkage Matrix](#business-write-back-linkage-matrix)).
...
The approval module publishes its domain events through the framework's transactional outbox transport (see [Event Bus](./infrastructure/event-bus)). Every approval command writes the event record in the same transaction as the business mutation; the outbox relay then forwards them to the configured sink.
...
Use lifecycle hooks for invariants that must hold inside the transaction (e.g. allocating a tightly-coupled business row). Use event subscriptions for everything else. `approval.NewFilteredLifecycleHook(hook, filters...)` wraps a hook with the same `InstanceFilter` vocabulary used by `SubscribeInstance` (below), so a hook can be scoped to specific flow codes or tenants without hand-written predicates.
...
`FlowVersion.FormSchema` is the host-owned designer document, stored and
returned verbatim (opaque `json.RawMessage`) — the framework never
interprets it. `FlowVersion.FormFields` is the flat `[]FormFieldDefinition`
list derived from `FormSchema` once at deploy through the injected
`FormSchemaParser`; it is the only form shape the framework itself consumes
(form-data validation, storage-table DDL, aggregate checks). Each
