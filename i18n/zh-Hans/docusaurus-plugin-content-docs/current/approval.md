...
RPC 调用使用 [API](./building-apis/api) 里的标准 envelope：`resource`、`action`、
...
`instance.formData` 中（已剔除当前查看者不可见的字段）；业务绑定流程的 opaque
业务引用在 `instance.businessRef` 中；`fieldPermissions` 是按查看者视角计算的
字段可交互性映射，客户端应原样应用。
...
> 老版本里归属 `[vef.approval]` 的 `outbox_relay_interval` / `outbox_max_retries` / `outbox_batch_size` 已在 v0.21 迁移至 `[vef.event.transports.outbox]`，由全框架统一的 outbox transport 服务所有模块——参考 [事件总线](./infrastructure/event-bus)。审批模块的 binding listener 和 outbox 发布两侧都会在启动时通过 `event.RouteInspector` 断言路由，路由配错时应用直接启动失败，而不是静默降级。
...
`BusinessInstanceIDField`、`BusinessStartedAtField` 和 `BusinessFinishedAtField` 联动列（见下方
[业务写回联动矩阵](#业务写回联动矩阵)）。
...
审批模块通过框架统一的事务性 outbox transport 发布领域事件（见 [事件总线](./infrastructure/event-bus)）。每条审批命令都把事件记录写在和业务变更相同的事务里，再由 outbox relay 转发到配置的 sink。
...
事务内必须成立的不变量（比如分配一个紧耦合的业务行）应该用 lifecycle hook；其他场景都用事件订阅。`approval.NewFilteredLifecycleHook(hook, filters...)` 用与 `SubscribeInstance`（见下文）相同的 `InstanceFilter` 词汇包装一个 hook，让 hook 可以按 flow code 或 tenant 限定范围，而不必手写谓词。
...
业务绑定的流程配置 `Flow.BusinessTable` / `BusinessPKField` / `BusinessStatusField`（必填），以及三个可选列——`BusinessInstanceIDField`、`BusinessStartedAtField`、`BusinessFinishedAtField`（Go 字段名；JSON 仍保持 `businessPkField` 等命名）。为 `nil` 表示"永远不动那一列"。引擎拥有的写回（`binding.Writer.WriteBack`）会按 `approval.BindingTrigger` 投影实例的当前状态：
...
`column` 指定要聚合的数字列（`sum` / `avg` 必填，`count` 禁止填写）。
...
`FlowVersion.FormSchema` 是宿主自有的设计器文档，原样存储、原样返回（不透明的
`json.RawMessage`）——框架从不解析它。`FlowVersion.FormFields` 是部署时通过注入的
`FormSchemaParser` 从 `FormSchema` 一次性推导出的扁平 `[]FormFieldDefinition`
列表；它是框架自身唯一会消费的表单形状（表单数据校验、storage-table DDL、聚合校验）。
