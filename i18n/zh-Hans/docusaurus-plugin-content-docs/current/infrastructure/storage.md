...
`result.Err` 业务错误（经 API envelope 承载，`ErrCode*` 常量编号范围
`2200-2299`）：
...
其余上传流程业务错误遵循同一模式——每个 `ErrCode*` 常量对应一个同名的
`result.Err` 值：`ClaimNotPending`、`ClaimExpired`、
`UploadSizeExceedsLimit`、`MultipartNotSupported`、
`PublicUploadsNotAllowed`、`UploadTooManyParts`、`TooManyPendingUploads`、
`UploadRequiresMultipart`、`UploadRequiresFile`、`ClaimNotMultipart`、
`UploadPartNumberOutOfRange`、`UploadPartTooLarge`、`UploadPartTooSmall`、
`UploadPartsIncomplete`、`UploadObjectNotFound`、`UploadSizeMismatch` 与
`AbortFailed`。
