...
精确的 processor 类型声明——单条、批量与导出/导入——只在一处维护：
[泛型 CRUD 的 Processor 类型签名](./crud#processor-类型签名)。本页聚焦于
"什么时候用哪个 hook"以及事务内到底执行了什么。
...
因此 hook 与文件生命周期共享同一条事务；真正的后端删除则由 storage delete worker 异步执行（见 [存储](../infrastructure/storage)）。
...
继续阅读 [验证](../building-apis/validation) 和 [错误处理](../building-apis/results-and-errors)，看请求失败和业务失败是如何向客户端暴露的。
