...
:::tip
先阅读 [项目结构](../getting-started/project-structure) 了解基础目录组织，再阅读本页掌握命名、API 组织和项目层面的强制规则。
:::
...
- [路由](../building-apis/routing)：请求标识结构与路由暴露方式
- [自定义处理器](../building-apis/custom-handlers)：RPC action 到 handler 方法的解析规则
- [模型](../data-access/models)：字段名、JSON tag 与列名之间的配合方式
