...
`tabular:"name=ID;order=1"` 会被解析成一个 `name` 值，而不是 name 加 order。
...
`NewSchemaFromSpecs` 会立即校验输入：缺 `Key`、缺 `Type`、`Key` 重复、解析后的
`Name` 重复都会在构造期返回错误（`tabular.ErrMissingColumnKey`、
`ErrMissingColumnType`、`ErrDuplicateColumnKey`、`ErrDuplicateHeaderName`）。
...
Excel option marker 类型是 `excel.ExportOption` 与 `excel.ImportOption`。

| 选项 | 默认值 | 用途 |
| --- | --- | --- |
| `excel.WithSheetName(name)` | `Sheet1` | 导出工作表名 |
| `excel.WithImportSheetName(name)` | 无 | 按名称读取工作表 |
| `excel.WithImportSheetIndex(i)` | `0` | 按索引读取工作表（负数或越界返回 `excel.ErrSheetIndexOutOfRange`） |
| `excel.WithSkipRows(n)` | `0` | 读取前跳过 n 行 |
| `excel.WithoutHeader()` | 含 header | 第一行作为数据，按位置映射 |
...
`FormatterFn` 时，会写入 native typed cell；整数、浮点数、布尔值、`time.Time`、
`timex.Date` 和 `timex.DateTime` 在 Excel 里仍可排序或求和。带显式 `Format`、
`Formatter` 或 `FormatterFn` 的列会按文本写入。`timex.Time` 会保留为文本，避免生成
带 zero-date 的错误 Excel 单元格。
...
| CSV 包专属错误哨兵 | `tabular.ErrDataMustBeSlice` 等共享错误 |
