...
当前 storage 包审计在生成的 API ledger 中锁定 **181 public storage
entries**。分组 member surface 覆盖 **81 grouped storage field/method
entries**，分布在 **29 storage receiver/type families** 中：其中包含 **50
exported storage field entries** 和 **31 exported storage method entries**。
生成的公开 API 索引仍是完整签名清单；本页负责说明这些语义家族和面向用户的运行时行为。
