...
Zen-backed engine，并把它提供给 API handler 注入和 mold 字段变换。
...
因为 `zen-go` 基于 cgo，使用默认框架运行时的项目必须在
`CGO_ENABLED=1` 且 C toolchain 可用的环境下构建。
...
- `Compile` 返回可复用的 `Program`；后端可能把校验延迟到 `Run`。
- `env` 是变量环境，可以是 map，也可以是带 JSON tag 的 struct。
- Context cancellation 是 best-effort。当前 Zen backend 会在同步 cgo 调用
  开始前尊重已经取消的 context，但无法中断已经开始的 evaluation。
...
应用代码应该依赖 `expression.Engine`，不要依赖内部 Zen adapter。
...
在当前 Zen backend 下，格式错误的表达式可能被 `Compile(...)` 接受，并在
之后的 `Program.Run(...)` 中失败。
