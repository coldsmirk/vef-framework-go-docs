...
审查说明：本页覆盖 49 public password entries，其中包括 3 grouped password method entries，分布在 1 password receiver/type family；成组 encoder surface 包含 0 exported password field entries 和 3 exported password method entries。
...
`NewCipherEncoder` 即使收到 nil cipher 或 nil inner encoder，也会返回一个
`Encoder` wrapper。`Encode` 会返回 `ErrCipherRequired` 或 `ErrEncoderRequired`；
`Matches` 在依赖为 nil 或解密失败时返回 `false`；`UpgradeEncoding` 委托给内部
encoder，内部 encoder 缺失时返回 `false`。
...
| `ErrCipherRequired` | `NewCipherEncoder` 收到 nil cipher |
| `ErrEncoderRequired` | `NewCipherEncoder` 收到 nil inner encoder |
