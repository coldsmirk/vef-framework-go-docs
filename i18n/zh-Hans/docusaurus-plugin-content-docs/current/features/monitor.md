...
Grouped-family audit 固定了 188 grouped monitor field/method entries，覆盖
22 monitor DTO/service families：其中 179 exported DTO fields、9 exported
service methods。这些 entries 覆盖 JSON response shapes、service interface、
build metadata、runtime summaries 和 sample-backed metrics；verifier 会锁定排序后的
签名和 receiver/type 分布。
