...
| `cache.NewInvalidating[T](loader cache.KeyedLoaderFunc[T], logger logx.Logger) *cache.Invalidating[T]` | 创建 invalidating cache；调用方必须传入非 nil loader 和 logger |
...
`cache.NewInvalidating[T](loader, logger)` 包装的是内部 `cache.NewMemory[T]()` 实例。它不接收 TTL 或 Redis option。请传入非 nil 的 `cache.KeyedLoaderFunc[T]` 和非 nil 的 `logx.Logger`；构造器不会在后续方法调用前替你校验它们。
