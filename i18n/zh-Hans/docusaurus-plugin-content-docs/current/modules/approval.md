...
| Action | 请求字段 |
| --- | --- |
| `start` | `params.tenantId` 必填、`params.flowCode` 必填、`params.businessRef`、`params.formData` |
| `process_task` | `params.taskId` 必填、`params.action` 必填（`approve`、`reject`、`transfer`、`rollback` 或 `handle`）、`params.opinion` 最多 2000 字符、`params.formData`、`params.attachments`、`params.transferToId`、`params.targetNodeId` |
...
当 `Flow.BindingMode == BindingBusiness` 时，流程会携带 SQL 标识符（`BusinessTable`、`BusinessPkField`、`BusinessTitleField`、`BusinessStatusField`），引擎拥有的写回会把它们直接拼到 `UPDATE` 模板里。为防止 SQL 注入，框架按 `^[A-Za-z_][A-Za-z0-9_]{0,62}$` 白名单校验：
