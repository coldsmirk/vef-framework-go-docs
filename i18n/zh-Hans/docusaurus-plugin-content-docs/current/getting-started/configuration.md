...
这些段都直接映射到公开 `config` 包和内部模块构造函数。完整已审的 `config` public surface，包括导出结构、字段、方法和 fingerprint，见 [配置参考](../reference/configuration-reference)。
...
> 老版本里的 `outbox_relay_interval` / `outbox_max_retries` / `outbox_batch_size` 已经从 `[vef.approval]` 搬到 `[vef.event.transports.outbox]`，由全框架共享的 outbox transport 服务所有模块——参考 [事件总线](../features/event-bus)。
